...
Lapis is an open-source, self-deployable Cloudflare application that provides an auth-gated Web Vault backed by R2 for latest vault content, Artifacts for sealed Git history, Durable Objects for serialized vault coordination, and D1 FTS for simple keyword search. A companion Obsidian plugin connects Local Vaults to the Web Vault through a device-code flow and keeps content synchronized with patch-based file revisions, server-side merging, live notifications, and recoverable Conflict Notes.
...
47. As a Vault Owner, I want deleting a Conflict Note to mark it resolved, so that resolution works naturally in Obsidian.
48. As a Vault Owner, I want lightweight presence for active sessions/devices, so that I understand where my vault is open.
49. As a Vault Owner, I want a warning when two sessions are on the same file, so that I can avoid avoidable conflicts.
50. As a Vault Owner, I want no editing locks, so that I can keep working even if another session is open.
51. As a Vault Owner, I want every sealed version stored as a Git commit in Artifacts, so that my vault has durable version history.
52. As a Vault Owner, I want live Web Vault changes to sync before they are sealed, so that the product feels responsive.
53. As a Vault Owner, I want live revisions sealed after a short debounce, so that history remains useful without every tiny edit becoming a commit.
54. As a Vault Owner, I want a vault-level timeline of sealed commits, so that I can understand historical changes.
55. As a Vault Owner, I want whole-vault restore from an older commit, so that I can recover from large mistakes.
56. As a Vault Owner, I want per-file restore to create a new commit with older file content, so that restore remains append-only.
57. As a Vault Owner, I want latest vault browsing to read from R2, so that the Web Vault is fast and simple.
58. As a Vault Owner, I want Artifacts tokens hidden from normal clients, so that clients cannot bypass the product sync API.
59. As a self-hosting operator, I want Lapis deployable to a single Cloudflare account, so that I can run my own instance.
60. As a self-hosting operator, I want clear setup docs for Workers, Durable Objects, R2, Artifacts, and D1, so that deployment is reproducible.
61. As a self-hosting operator, I want no built-in billing or product quotas, so that my instance is governed by my Cloudflare limits and optional config.
62. As a self-hosting operator, I want optional upload/storage limits in configuration, so that I can protect my own deployment.
63. As a self-hosting operator, I want to export the latest vault as a zip, so that I can leave or back up outside Lapis.
...
- The current browsing state is materialized in R2. Artifacts stores sealed Git history. Durable Objects serialize vault mutations and hold vault coordination state. D1 FTS provides simple keyword search.
- Accepted changes update R2 immediately as live revisions and are sealed into Artifacts after a 2-10 second debounce.
...
- Conflict Notes are Markdown files under `.sync-conflicts/`, include full server/client/base context, and are resolved by deletion.
...
- Test external behavior at product seams: sync API, Web Vault reads, plugin connection flows, search results, conflict creation, restore behavior, and R2/Artifacts projection consistency.
- Sync tests should verify observable state transitions: accepted patch updates R2, emits notifications, updates search/backlinks, and eventually seals a commit.
